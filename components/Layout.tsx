...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col font-body overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-40 w-full glass-card border-b border-white/10 px-4 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          {/* Brand Logo */}
          <div 
            className="font-display font-bold text-sm sm:text-base md:text-lg text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-fuchsia-500 cursor-pointer truncate max-w-[140px] sm:max-w-xs md:max-w-none"
            onClick={() => navigate('/')}
          ...
          <div className="flex items-center gap-3 md:gap-4 shrink-0">
            ...
            <div className={`flex items-center gap-3 ${!isLandingPage && 'border-r border-white/10 pr-3 mr-1'}`}>
              <button onClick={() => navigate('/')} className="flex items-center gap-1 text-sm text-slate-300 hover:text-cyan-400 transition-colors">
            ...
            {user && !isLandingPage && (
              <>
                ...
                <div className="hidden lg:flex items-center gap-2 px-3 py-1 rounded bg-slate-900/80 border border-red-500/30 text-red-400 font-mono text-sm shadow-[0_0_10px_rgba(239,68,68,0.2)]" title="Session Time Remaining">
                ...
                <div className="h-6 w-px bg-white/20 mx-1 md:mx-2"></div>
      ...
      {/* User Profile Modal (Only relevant if user exists) */}
      {user && isProfileOpen && (
           ...
           <div className="bg-slate-900 w-full max-w-md border border-cyan-500/50 rounded-2xl shadow-[0_0_50px_rgba(6,182,212,0.2)] overflow-hidden">
              <div className="bg-gradient-to-r from-cyan-900/40 to-slate-900 p-6 flex justify-between items-start border-b border-white/10">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <h2 className="font-display font-bold text-xl text-white uppercase tracking-wide">{user.name}</h2>
                ...
                <div className="bg-slate-950 p-4 rounded-lg border border-white/5 space-y-3">
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <div>
                         ...
                         <div className="font-mono text-white">{user.location?.latitude.toFixed(6)}</div>
                      </div>
                      <div>
                         <div className="text-[10px] text-slate-500">Longitude</div>
                         <div className="font-mono text-white">{user.location?.longitude.toFixed(6)}</div>
                      </div>
                   </div>
                </div>

                {/* Session Timer (Visible in Modal too for Mobile) */}
                <div className="bg-red-950/20 p-4 rounded-lg border border-red-500/20 flex items-center justify-between">
                   <div className="flex items-center gap-2 text-red-400 text-sm">
                      <Clock size={16} /> Session Expires In:
                   </div>
